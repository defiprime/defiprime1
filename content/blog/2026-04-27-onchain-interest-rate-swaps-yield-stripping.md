...
url: /onchain-interest-rate-swaps-yield-stripping.html
...
![On-chain IRS protocols by TVL in April 2026: Pendle at $1.43B, Exponent at $73M, IPOR Derivatives at $2M, Voltz at $238K (sunset). Log scale. Source: DefiLlama.](/images/blog/onchain-irs-current-tvl.png)

The chart sits on a log scale because a linear axis would render the bottom three bars as invisible slivers next to Pendle. Even after the 2026 unwind, Pendle is roughly 20x Exponent and three orders of magnitude larger than the synthetic IRS protocols. Kairos's beta cumulative notional of around $300M does not appear on this axis because notional and TVL are not the same metric, but at the protocol's current size it would slot between Exponent and IPOR rather than rewriting the picture.

[Pendle](/derivatives) on EVM is the dominant venue. Yield-bearing assets — [Lido](https://lido.fi/) stETH, [Ethena](https://ethena.fi/) USDe, Aave aUSDC, [Morpho](https://morpho.org/) vault positions, restaking LRTs, Treasury-backed RWAs — get deposited into Pendle and split into PT (principal token, fixed yield to maturity) and YT (yield token, floating yield through expiry). Both trade on a time-aware AMM that adjusts liquidity as expiry approaches, with a complementary orderbook layer for tighter execution on the larger pairs. The franchise extends now into Boros, a separate module that tokenizes perpetual funding rates from major venues like Binance and Hyperliquid into leveraged long/short rate exposure. Boros is the closest thing in the Pendle stack to a classical IRS product, and it depends on [Chainlink](https://chain.link/) Data Standard feeds with TWAP smoothing for the off-chain rates it imports.

![Pendle TVL from January 2024 to April 2026, peaking at $13.4B in September 2025 and unwinding to $1.43B by late April 2026. Source: DefiLlama.](/images/blog/onchain-irs-pendle-trajectory.png)

The TVL shape across the cycle matters for how to read the current number. Pendle climbed from roughly $200M at the start of 2024 to a $6.7B local peak in mid-2024 (the LRT season), pulled back to ~$3B late in the year, then ramped through 2025 to a $13.4B peak in September on USDe, eUSDe, and curated stablecoin yield positions. The unwind since October 2025 is the natural roll-off as those expiries matured and the next round of demand failed to replace them at the same scale. The current $1.43B is well above the early-2024 baseline and well below the cycle peak, which is the relevant frame for sizing the rest of the category against it.

[Exponent](https://www.exponent.finance/) is the Solana counterpart, often described as the Pendle of Solana, though the architecture leans more on a hybrid orderbook-to-CLMM design than on a time-aware AMM. Users wrap variable-yield Solana DeFi positions — [Jito](https://www.jito.network/) staking (JitoSOL), Marinade (mSOL), [Kamino](https://kamino.finance/) lending and liquidity, Marginfi credit — into income tokens and yield tokens, then trade for fixed-rate exposure or leveraged variable-rate bets. The on-record figures the team highlights are roughly $2B in cumulative yield-volume traded, $250M-plus of yield settled to users, 35,000+ users, sub-second median execution, and zero security incidents since its 2024 launch. Current TVL on DefiLlama sits around $73M, off a July 2025 peak of $133M. A v2 is in private beta. Pendle is also expanding onto Solana through PT issuance against Kamino positions with Chainlink CCIP wiring, which sets up the first real cross-chain rate-trading footprint.

[Kairos](https://kairosswap.com/) is the active synthetic IRS attempt. The protocol launched on Ethereum mainnet in early 2026 with a beta on Base and raised $2.4M in seed funding in March. The pitch is permissionless rate-market creation: anyone can open a swap market against any oracle-supported reference rate (Aave borrow rate, Morpho vault APY, Lido stETH yield, RWA cash flows, off-chain benchmarks routed through [Pyth](https://www.pyth.network/) or Stork), set tenor and collateral parameters, and let traders take leveraged fixed-vs-floating positions, with capital efficiency the team describes as up to roughly 5,000x notional-to-collateral. Positions are ERC-721 NFTs that compose into vaults and structured products downstream. Beta cumulative notional is reported at over $300M. That is real activity for a six-month-old protocol and small relative to a single day on Pendle.

[IPOR](https://www.ipor.io/) is the longer-running synthetic IRS protocol on EVM, live since 2022 on Ethereum, [Arbitrum](/arbitrum), and [Base](/base). It runs an AMM-based fixed-vs-floating market on top of its own decentralized rate index (the IPOR Index), originally calibrated against blue-chip stablecoin lending rates. The IPOR Derivatives book on DefiLlama shows roughly $1.6M in TVL as of late April 2026, off an early-2023 peak of around $44M. The protocol has shipped continuous improvements and has institutional partners, but it has not crossed the threshold where rate-trading flow finds it organically. Notably, IPOR weathered a mitigated oracle manipulation event early in its life when a large position spiked the underlying index — a reminder that synthetic IRS is structurally more oracle-dependent than yield stripping is.
...
![On-chain rate-protocol TVL on a log scale from June 2022 to April 2026: Pendle climbs into the billions in 2024 while IPOR and Voltz peak below $50M and decline. Exponent on Solana enters in 2025 and tracks an order of magnitude below Pendle. Source: DefiLlama.](/images/blog/onchain-irs-divergence.png)

The chart above is the divergence in one frame. IPOR and Voltz both peaked in early 2023 at roughly $30–45M, set what turned out to be the synthetic-IRS ceiling for the cycle, and have declined steadily since. Pendle was at the same scale in early 2023, then opened a ~100x gap by mid-2024 and held it through the cycle peak. Exponent showed up on Solana in 2025 at the same order of magnitude as the surviving synthetic IRS protocols and has tracked closer to Pendle's category than to theirs ever since. Two designs starting from the same baseline; one design captured the next two cycles.
