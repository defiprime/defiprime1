    ...
    def amp_images(input, responsive = true, wi = nil, he = nil)
      ...
      doc.css('img:not([width])').each do |image|
        if wi && he
          ...
          if image.attr('src') && image.attr('src').start_with?('http://', 'https://')
            src = image.attr('src')
          elsif  image.attr('data-src') && image.attr('data-src').start_with?('http://', 'https://')
            src = image.attr('data-src')
          else
            # FastImage doesn't seem to handle local paths when used with Jekyll
            # so let's just force the path
            src = File.join(Dir.pwd, '', image.attr('src')) #fix bug described here  https://github.com/juusaw/amp-jekyll/issues/24#issuecomment-365681698
          end
