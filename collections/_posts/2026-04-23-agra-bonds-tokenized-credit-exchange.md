...
## The Contract Layer: Strong Token, Opaque Exchange

The asset side of Agra's stack is straightforward because Agra did not build it. The ACRDX share token at `0x9477724Bb54AD5417de8Baff29e59DF3fB4DA74f` is a verified Solidity 0.8.28 ShareToken deployed by an Anemoy-linked address in late 2025. It is a standard Centrifuge-style RWA primitive: ERC-20 with EIP-2612 permits, a MakerDAO-style `wards` authorization pattern for mint/burn and config, transfer-restriction hooks that enforce KYC on-chain, and a vault mapping that links the share token to the underlying Centrifuge pool for NAV reference. On-chain activity to date is almost entirely approvals, which is consistent with a fund share that has not yet seen meaningful secondary turnover.

The design is centralized by construction, and deliberately so. Authorized operators can mint, burn, pause transfers via the hook, and reconfigure the vault pointer. That is the correct trust model for regulated private credit. You want the issuer to be able to enforce redemptions, blacklist sanctioned addresses, and pause if the underlying fund hits a gate, and it is what every credible RWA token ships with. It is not, however, what DeFi users are used to evaluating, and the lack of a global pause (the contract relies on `deny` to revoke privileges rather than a circuit breaker) is worth noting.

The exchange side is less transparent. Each Agra market lives behind an ERC-1967 proxy address referenced in the UI as a `market` query parameter, and those proxies handle order placement, matching, and settlement against the underlying share token and USDC. Source code for the orderbook contracts is not verified on Etherscan as of this writing, no public audit reports have been posted, and there is no public GitHub repository for the matching logic. Settlement is on-chain and non-custodial, with funds moving directly between maker and taker, but the path those funds take through the proxy is currently a black box to anyone who did not write it.

For a beta-stage product that has not yet advertised any institutional capital, that is not unusual. For anyone considering using the venue at size, it is the single biggest open question. The difference between a clean CLOB implementation and a subtly broken one rarely shows up in the UI; it shows up in an edge case six months in.
