...
Papertrade is a synthetic perpetuals protocol on Hyperliquid where the liquidity pool starts at zero. No founders' deposit, no upfront raise, no LP onboarding flow of any kind. The pool fills only from cumulative user losses, and the protocol's native token, PAPER, is minted exclusively to traders who close at a loss or get liquidated. Winners get paid when the LP has the USDC to pay them; when it doesn't, the profit portion of their trade goes into a FIFO queue and clears as future losses refill the pool. The mechanism is set out in a [whitepaper](https://docs.papertrade.xyz/martingaler-whitepaper.pdf) called *Martin Galer: A Bootstrapping Mechanism for Sustainable Asynchronous Speculation Games of Chance*, co-authored by Jez (@izebel_eth) and Blurr, the latter known on-chain as "Refund Contract Deployer."

That is a coherent piece of onchain mechanism design and a contribution worth working through carefully. Bankroll requirements have killed or distorted every prior onchain casino in the lineage — SatoshiDice, Bustabit, Etheroll, EtherCrash — and the Martin Galer payout queue is the cleanest answer to that problem so far. The PAPER tokenomics are structurally tighter than most 2024-2025 "fair launch" attempts, with a mint path that depends entirely on real trading losses and no insider allocations behind a vesting cliff. Run on top of Hyperliquid's BBO precompile, the whole thing is a self-contained leveraged speculation venue that doesn't need its own price oracle, its own matching engine, or its own seed liquidity.

It is also a leveraged speculation product. The marketing emphasises "1000x leverage" and "perpetual futures exchange." The whitepaper calls it a "speculation game of chance" and the docs are explicit about the house edge, the asymmetric payout curve, and the fact that net protocol equity can go negative. Both descriptions are accurate. They land in different regulatory categories, and the framing choices the team has made — anonymous authors, "perp DEX" marketing on the front page, "speculation game" language buried in the whitepaper — are doing work. Working through the design also means working through what those choices imply.
...
Four metrics will tell you whether the design works as the docs describe.

The first ~$2M of LP growth is the flat-rate window. The rate at which the LP fills through this window is the realised house edge — the speed at which 100-PAPER/$1 mints accumulate against actual user losses. A slow fill means the user base is closer to break-even than the design implies. A fast fill confirms the design works as advertised, which is good for early stakers and bad for traders.

The queue overhang ratio is the user-experience proxy. The closer queue total runs to LP balance, the more often winning closes will wait. Early protocol life will look queue-heavy by definition; the question is how quickly it clears once aggregate flow tips positive-EV for the LP.

The first owner action through the 7-day timelock tells you what protocol governance looks like in practice. A trivial parameter tweak is the boring outcome. An impact-curve recalibration in either direction is informative. A mint-curve constant change is a serious signal about how the team treats the existing PAPER holder base.

The regulatory surface will move on its own clock — either through a de-anonymization tied to an institutional relationship, a regulator naming the product and forcing a structural response, or a long silence that becomes its own answer.

The protocol is a more carefully reasoned piece of casino engineering than most onchain casinos, and a leveraged speculation product whose regulatory positioning depends on no one calling it that out loud. Both descriptions track. Once trading goes live, the LP curve and the queue dynamics will say which one is doing more of the work.
