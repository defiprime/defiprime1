...
This piece reflects the state of things as of July 2, 2026, roughly a day after mainnet went live. Almost every hard number here is either a testnet figure, a launch-materials claim, or a figure circulating on crypto Twitter that has not been independently confirmed on-chain. Treat all of it as a snapshot of an ecosystem that is hours old, not a settled picture.
...
| Trading / DEX | [Uniswap](/exchanges) (dedicated AMM), Pleiades (prop-trading AMM), Lighter (perps and spot), 0x, 1inch |
...
The tokens are not shares. They are tokenized debt securities issued by Robinhood Assets (Jersey) Limited, a subsidiary in the Channel Islands. Holding one gives you price exposure to the underlying stock. It does not give you ownership of the stock, voting rights, or a direct claim on the company. What you hold is a liability of a Robinhood entity that promises to track the price. If that entity fails, you are a creditor, not a shareholder. This is the same structural model that drew public pushback when earlier tokenized-equity efforts launched, including OpenAI distancing itself from tokens that claimed to represent its equity.

Then there is geography. The tokenized stocks are available in 120-plus countries but explicitly not in the United States, and they are restricted in the UK, Canada, Switzerland, and elsewhere. Robinhood's core audience is American retail. The flagship product on its new chain is, by design, unavailable to most of the people who made Robinhood a household name. Robinhood is rolling out an onchain lending product (Robinhood Earn) for eligible US users, so Americans get a version of the DeFi story, just not the tokenized-equity part that the chain was built around.
...
The primary trading venues on day one are automated market makers: [Uniswap](/exchanges)'s dedicated AMM and Pleiades, a proprietary AMM for prop trading. AMMs are excellent for long-tail crypto assets. They are a debatable choice for equities, where price discovery has run on central limit order books for a century and where the reference price updates continuously during market hours. An AMM holding tokenized NVDA has to be arbitraged back to the real quote constantly, and the people doing that arbitrage extract value from the liquidity providers on the other side. Thin books plus a fast-moving external reference price is a recipe for LPs getting picked off, which in turn thins the books further.

This is the same critique that gets leveled at on-chain perps venues that lean on AMMs instead of order books, and it is why order-book designs keep winning liquidity in that category. Whether tokenized equities need a proper CLOB to trade well at size is an open question, and Robinhood Chain launching AMM-first is a real bet, not a solved problem. Early liquidity on tokenized stocks is exactly the metric to watch over the coming weeks, and it is exactly the metric nobody can cite yet.
...
Two other things feed the skepticism. Robinhood reportedly cut hundreds of staff not long before this launch, which reads oddly against the execution demands of standing up a chain, an issuer, a wallet integration, and a lending product simultaneously. And the incentive to bootstrap activity, reportedly around 90 days of covered gas fees, is the kind of subsidy that inflates early usage numbers and then expires. None of this is disqualifying. All of it is reason to discount the day-one metrics.
...
| Gas subsidy | ~90 days of covered fees | Reported, unconfirmed terms |
| Lighter incentives | ~$11M in LIT for perps liquidity | Ecosystem-project claim |
| Robinhood Earn yield | ~7% APY lending USDG via Morpho, with Lloyd's/RELM insurance on smart-contract risk | Launch-materials claim |
| $HOOD reaction | ~8% on the announcement | Reported market move, not a chain metric |
