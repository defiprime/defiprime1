...
permalink: stock-paired-memecoins
h1title: "Inside the Stock-Paired Memecoin Boom"
pagetitle: "Quoted in Nvidia: Inside the Stock-Paired Memecoin Boom"
metadescription: "Memecoins on Robinhood Chain are priced in tokenized shares, not dollars. Our own read finds 432 such pools holding 17% of the onchain float of 19 stock tokens."
category: blog
featured-image: /images/blog/stock-paired-memecoins-ogp.png
intro: "A memecoin whose quote currency is a share of Nvidia is no longer a curiosity on Robinhood Chain. Reading the pools directly, we find 432 stock-quoted memecoin markets holding 17% of the onchain float across 19 tokenized equities and routing 31% of their trading volume."
author: sawinyh
tags: ["Analysis", "RWAs"]
---

The largest stock-paired memecoin on Robinhood Chain is not priced in dollars. Artificial Inu, ticker AI, trades in one pool against tokenized Nvidia, and that pool holds 8,783 NVDA tokens, which is 16.2% of every tokenized Nvidia share that exists on the chain. Buy the dog coin with stablecoins and the router buys Nvidia first, because Nvidia is where the depth is.

One clever launch would be a curiosity. This is now the default product of at least four launchpads, and the single largest identifiable use of tokenized equities on the chain.

We read the pools ourselves rather than take the launchpads' word for it. Across 19 of the most liquid Robinhood stock tokens, at block 51,651,897 (roughly 10:30 UTC on September 1, 2026), there are 432 live liquidity pools in which a tokenized equity is the quote asset for some other token. Those pools hold $8.84M of stock tokens against a total onchain float of $51.5M for the same 19 tickers, or 17.2%. In the last 24 hours they carried $95.3M of the $304.1M in DEX volume those tokens traded, or 31.3%.
...
The floats themselves are the context that makes those percentages possible. Tokenized Meta is 2,028 tokens, about $1.2M. Tokenized Palantir is 6,562 tokens, about $1.2M. These are not markets, they are inventories, and a single memecoin can be a meaningful fraction of one without doing anything unusual.

Reserves relative to volume are just as thin. The STARTUP/SPY pool turned over $3.5M in a day on 27.4 SPY tokens of inventory, roughly $21K worth. A pool that shallow reprices on almost any order, which is the fragility behind the weekend premium we get to further down.

The two largest pools are worth naming precisely. AI/NVDA holds 8,783 NVDA against a memecoin marked at roughly $188M, and it did $6.2M of volume in 24 hours. BONER/HIMS holds 37,172 HIMS on its own, 50.4% of every tokenized Hims & Hers token on the chain, against a $55M memecoin, and it did $12.5M. Long's own account [claimed on August 27](https://x.com/longdotxyz/status/2093097393608958360) that "23%+ of the total NVDA supply is now backing $AI," a broader definition than pool reserves that presumably includes its vault; our verified pool-only figure is 16.2%, and 24.9% once every NVDA-quoted pool on the chain is counted.

## The Memes Are Opinions About the Stock

The largest pair for each ticker gives the game away. SAYLORMOON against MicroStrategy. INCEL against Intel. CHIP against AMD. SLOP against Meta. SHORT against GameStop. OOF against Roblox. GOOGOL against Alphabet. SPACEHOOD against the SpaceX wrapper. BONER against the company that sells erectile dysfunction treatment.

The joke is about the company in every case, and the pool is how the punchline gets delivered. In the last memecoin cycle the reference point was a cat, a politician, or another token. Here it is an equity, and the coin says something about that equity, which is closer to an opinion than a lottery ticket.

It also means the namespace collides. There is a token on this chain called GameStop, ticker GME, marked at $4.06M, that is not Robinhood's tokenized GameStop; it is a memecoin quoted against Robinhood's tokenized GameStop, and it traded $652K in 24 hours. There is a "NVIDIA Robinhood Coin" with the ticker NVDA at $880K that is not the NVDA anyone means. A buyer reading a ticker rather than a contract address on this chain is one click from owning something entirely different from what they think.
...
Underneath the memecoin framing is a token whose treasury accumulates Nvidia exposure out of its own churn. Flap runs the same logic in a more literal form, paying holders a stream of tokenized stock out of fee revenue instead of banking it. Bankr, [per Foresight News](https://global.foresightnews.pro/article/16845), pays creator fees out in the paired stock token.

A dog coin paired against ETH converts speculative churn into ETH for whoever collects the fees. A dog coin paired against NVDA converts the same churn into a claim on a wrapper that tracks a real company, and it banks it in a token treasury rather than an individual's wallet. That is the part that is actually new here. Whether the vault stays intact when the token stops trading is the open question, and no one has been through that cycle yet.

The layer collecting the most from this is neither the memes nor the stocks. Uniswap V4 on Robinhood Chain earned $6.5M in fees in 24 hours, out of $14.3M chain-wide, per DefiLlama at the time of writing. The Pons launchpad's V2 contract earned $4.2M. Chain TVL reached $740M on September 1, up from $374M on August 1, and DEX volume on the last complete day, August 31, was $1.49B.
...
Follow the stack. A share of Nvidia, wrapped as a debt security by a broker, wrapped again as a 3x perpetual position on a separate venue, wrapped a third time as an ERC-20, and used as the pricing asset for a joke token. Each wrapper adds a peg that holds only while its own maintenance mechanism is awake, and the weekend problem that produced the HIMS squeeze applies to every one of them at once.
...
The vaults are the other unresolved question. Every stock-paired launch promises that fee revenue accumulates into stock tokens held permanently, and none of these projects has yet gone through the part of the cycle where the memecoin stops trading and somebody has to decide what the vault is for. Until one does, the treasury claim is a design, not a result.

And the target selection is worth tracking, because small floats are what make a corner cheap. Of the nineteen tickers we read, nine carry less than $1.5M of onchain float, seven of them in the table above. The only real fix on the issuer's side is carrying inventory across the weekend, which costs money and which nothing in the supply data suggests anyone is doing yet.

The most successful use anyone has found for tokenized equities so far is as the denominator of a memecoin. That was not the plan. It is also, at the same time, producing genuine fee revenue, putting equity wrappers into a couple of hundred thousand wallets, and breaking the wrapper's price every weekend.
