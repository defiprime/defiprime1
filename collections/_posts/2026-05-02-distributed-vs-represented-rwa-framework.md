...
That reclassification is the most useful thing to happen to RWA data in two years, and it is also the moment to look hard at what the framework does and does not tell you. The two-bucket split is a clean lens on transferability. It is not a lens on liquidity, on regulatory fit, or on whether any of these assets are actually being used the way the tokenization pitch promised.

## What the Framework Actually Says

The taxonomy is deliberately narrow. rwa.xyz built it with issuers, and the entire point was to stop having philosophical arguments about what counts as "on-chain." Instead of debating wrappers, custody arrangements, or chain choice, the framework reduces the question to two on-chain attributes that anyone can verify by reading the contract.
...
The split also reveals what the old aggregate was hiding. The Canton Network alone holds something like $380B in tokenized repos, settling between large institutional counterparties on a permissioned chain. Those repos are real, useful, and structurally important to TradFi rails, but they are not assets that can move into a [DeFi yield strategy](/defi-yield-risk-premium), get posted as collateral on [Aave](/aave), or trade on a public DEX. Counting them in a number compared against ~$30B of mobile, transferable on-chain assets was a category error, and the new dashboard fixes it.
...
- **Private credit** shows roughly $5B Distributed against $18 to $19B in the broader category, with most of the gap sitting in Represented vehicles. On the Distributed side, the visible names include Apollo's ACRED wrappers, Maple's syrupUSDC, and the Centrifuge issuer set. The Represented slice is dominated by Figure's HELOC stack and large institutional credit platforms.
...
- **Private equity** is a small dollar number and the fastest mover, up north of 160% over the trailing 30 days as new tokenized fund vehicles come online.

Two patterns matter here. First, the categories that are big in dollars (private credit, repo) are heavy on Represented. Second, the categories that are growing fastest in percentage terms (private equity, stocks) are heavy on Distributed but tiny in absolute size. The Distributed total is up roughly 5% over the last 30 days, which is healthy for an institutional asset class but not the kind of growth that gets a category from $30B to $100B inside a year without a step change in either Treasuries or private credit.
...
The default explanation for why an asset is Represented is "regulation," and that is the right answer most of the time, but it is not the only one. The categories that stay locked tend to fail one or more of three tests.
...
The migration thesis the rwa.xyz team articulates is real, but it depends on which bucket each Represented asset is in. Regulatory-driven Represented assets flip when rules change, which is slow and uneven. Design-driven ones do not flip at all and probably should not. Operations-driven ones flip whenever the issuer puts engineering hours into the problem, which is usually fast once the decision is made.
...
The right read of the dashboard is the one the team seems to intend. Distributed is the share of tokenized assets that meet a basic mobility test. It is the floor for any further analysis, not the conclusion. Combined with category-level breakdowns, on-chain transfer activity, secondary-venue depth, and the regulatory map for each asset type, it gives a much sharper picture than the old aggregate ever did. On its own it gives a number that is easy to misuse.

## What to Watch

Three signals over the next few quarters will tell us whether the Distributed total is going to scale toward the $100B number that gets quoted in 2026 forecasts.

The first is private credit migration. The single highest-leverage move available is for the largest Represented private credit platforms (Figure being the cleanest example) to complete the transition to Distributed. Even a partial flip on the Figure HELOC book alone would meaningfully change the Distributed total without any new origination. The Solana integration Figure has been pushing on is the visible piece of that migration, and how it scales over the rest of 2026 is the most important number in the category.
...
The third is whether the secondary venues clear real volume. Agra on the credit side, the standard DEXes on the Treasury side, and the [emerging tokenized-stock venues](/exchanges) on the equities side are the gating items. Distributed assets without active secondary markets are technically mobile and practically dormant. Volume on those venues is the most direct signal that the Distributed number means what the dashboard implies it means.

The framework itself is settled enough that "Distributed vs. Represented" is now standard vocabulary in serious RWA discussions. The work left is on the analysts using it. Treat Distributed as a useful first cut and a poor final answer, layer real liquidity and regulatory analysis on top, and the dashboard becomes one of the more honest data products in the category. Treat the headline number as the answer and the framework's biggest contribution gets squandered the same way the old aggregate did.
